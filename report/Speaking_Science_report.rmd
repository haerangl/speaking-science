...
We mentioned earlier that we observed a surprising amount of users reading our article in under one minute. While this may be an accurate model of how people interact with news articles general, we suspect there might be another issue preventing users from taking their time reading. We conducted the survey via Mechanical Turk, an online pay-per-task service provided by Amazon. Under Mechanical Turk (mTurk), workers are directly incentivized to finish tasks with reasonable quality as fast as possible. In other words, quickly finishing individual tasks means more tasks per hour, which achieves a higher per hour earning rate. As such, we believe that some of our respondents did not take the proper amount of time reading and answering questions. To account for this, we dropped a number of responses that we believe could not have answered each question earnestly. The methodology and complete reasoning were discussed in the compliance section. This adjustment was still our best guess, which makes us hesitate to say that we would expect to see an effect of similar magnitude in the general population.
